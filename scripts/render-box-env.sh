...
BOX_HOSTNAME="${CLAWFIRM_BOX_HOSTNAME:-${CLAWFIRM_BOX_ADAPTER_HOSTNAME:-box.$DOMAIN}}"
BOX_TOKEN="${CLAWFIRM_BOX_TOKEN:-${CLAWFIRM_BOX_ADAPTER_TOKEN:-${CLAWFIRM_BOX_API_KEY:-dev-box-token}}}"
...
python3 - <<'PY' "$OUT_PATH" "$PUBLIC_IP" "$BOX_HOSTNAME" "$BOX_TOKEN" "$TEMPLATE_PROFILE" "$FIREWALL_PROFILE" "$SWAP_STRATEGY"
import pathlib
import sys

path = pathlib.Path(sys.argv[1])
public_ip, daemon_hostname, api_key, template_profile, firewall_profile, swap_strategy = sys.argv[2:8]
text = path.read_text()
replacements = {
    'CLAWFIRM_DNS_DEFAULT_A=127.0.0.1': f'CLAWFIRM_DNS_DEFAULT_A={public_ip}',
    'CLAWFIRM_AUTH_DOMAIN=box.example.com': f'CLAWFIRM_AUTH_DOMAIN={daemon_hostname}',
    'CLAWFIRM_CADDY_MANAGED_HOST_EXCLUDE=box.example.com': f'CLAWFIRM_CADDY_MANAGED_HOST_EXCLUDE={daemon_hostname}',
    'CLAWFIRM_BOX_TOKEN=replace-with-strong-random-secret': f'CLAWFIRM_BOX_TOKEN={api_key}',
