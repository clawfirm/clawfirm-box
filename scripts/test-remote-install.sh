...
BOX_BASE_URL="${BOX_BASE_URL:-${ADAPTER_BASE_URL:-https://box.example.com}}"
BOX_SECRET="${BOX_SECRET:-${ADAPTER_SECRET:-}}"
BOX_SECRET="${BOX_SECRET:?set BOX_SECRET}"
...
curl -fsS "${BOX_BASE_URL}/health"

curl -fsS -X POST "${BOX_BASE_URL}/dns/ensure-zone" \
  -H "Authorization: Bearer ${BOX_SECRET}" \
  -H 'Content-Type: application/json' \
  -d "{\"zone\":\"${ZONE}\",\"requestedBy\":\"deploy-test\",\"reason\":\"bootstrap box zone\"}"

curl -fsS -X POST "${BOX_BASE_URL}/dns/apply-records" \
  -H "Authorization: Bearer ${BOX_SECRET}" \
  -H 'Content-Type: application/json' \
  -d "{\"zone\":\"${ZONE}\",\"requestedBy\":\"deploy-test\",\"reason\":\"point apex at box\",\"records\":[{\"type\":\"A\",\"name\":\"@\",\"value\":\"${A_RECORD}\",\"ttl\":300}]}"

curl -fsS -X POST "${BOX_BASE_URL}/dns/get-zone" \
  -H "Authorization: Bearer ${BOX_SECRET}" \
  -H 'Content-Type: application/json' \
  -d "{\"zone\":\"${ZONE}\"}"
